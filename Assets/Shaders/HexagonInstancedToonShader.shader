Shader "Custom/HexagonInstancedToonShader"
{
	Properties
	{
	...
	SubShader
	{
		pass {
			...
			v2f vert(appdata v) {
				...
				o.pos = UnityObjectToClipPos(v.vertex + float4(position, 0.0));//�л�����������
			...
			ENDCG
				...
				Pass {
					...
					v2f vert(appdata v) {
						v2f o;
						float3 position = _PositionBuffer[v.id]; // Read position from buffer
						o.pos = UnityObjectToClipPos(v.vertex + float4(position, 0.0));
						o.worldNormal = UnityObjectToWorldNormal(v.normal);
						o.worldPos = mul(unity_ObjectToWorld, v.vertex + float4(position, 0.0)).xyz;
						// 计算阴影数据
